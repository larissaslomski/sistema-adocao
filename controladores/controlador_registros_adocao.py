...
class ControladorRegistrosAdocao():
    ...
    def verifica_tam_apartamento(self, adotante, animal):
    ...
    def incluir_registro_adocao(self):
        ...
        if cachorro_ou_gato == 1:
            ...
            if adotante is not None and cachorro is not None:
                ...
                tam_apartamento = self.verifica_tam_apartamento(
                    adotante, cachorro)
                if tam_apartamento:
                    if maior_idade:
                        if tomou_vacinas:
    ...
    def alterar_registro_adocao(self):
        self.listar_registro_adocao()
        cod_registro_adocao = self.__tela_registro_adocao.seleciona_registro_adocao()
        registro_adocao = self.pega_registro_adocao_por_codigo(
            cod_registro_adocao)
        if registro_adocao is not None:
            while True:
                assinatura = self.__tela_registro_adocao.assina_termo_responsabilidade()
                if assinatura.upper() == "S":
                    ...
                    self.__tela_registro_adocao.mostra_mensagem(
                        "Você deve assinar o termo para seguir com a adoção.")
                    self.retornar()
        else:
            self.__tela_registro_adocao.mostra_mensagem(
    ...
    def abre_tela(self):
        lista_opcoes = {1: self.incluir_registro_adocao, 2: self.listar_registro_adocao, 3: self.excluir_registro_adocao,
                        4: self.alterar_registro_adocao, 5: self.gerar_relatorio_adocao, 0: self.retornar}
